...
class DepositCommand extends BaseCommand
{
    ...
    {
        parent::__construct('deposit', $plugin);

        $this->setDescription('Deposit money into your bank');
    }
        ...
        if ($this->getOwningPlugin()->getPlayerManager()->canDoTransactions($sender)) {
            $this->getOwningPlugin()->getPlayerData()->loadValue($sender->getName(), PlayerData::BANK_MONEY, function (int $bankMoney) use ($sender): void {
                if ($sender->isConnected()) {
                    ...
                    EconomyForms::sendDepositMenu($sender, $content, $bankMoney, $this->getOwningPlugin());
                }
            });
        } else {
            $sender->sendMessage(MMOPlugin::getPrefix() . TextFormat::RED . "You can't do transaction right now!");
        }
